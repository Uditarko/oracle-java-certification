...
import my.learning.java.streams.advanced.Category;
...
public class AdvancedStreamsExercise03 {
     ...
     * @param expensiveLimit Products more expensive than this are considered
     * expensive.
     * @return A map containing the cheap, medium and expensive products in
     * separate lists.
     */
    public Map<PriceRange, List<Product>> groupProductsByPriceRange(List<Product> products, BigDecimal cheapLimit, BigDecimal expensiveLimit) {
        ...
        Function<Product, PriceRange> classifier = (p) -> {
        ...
        return products.stream().collect(Collectors.groupingBy(classifier, Collectors.toList()));
    }
}
